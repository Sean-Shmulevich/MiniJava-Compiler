...
/*
The symbolTable is a list of all symbols in the program.  A symbol table entry
is a map from attribute IDs to attribule values.  In addition to symbolTable,
there is scopeStack that keeps track of current active scopes.  A scope is
defined by the "bottom" index where the scope begins in the symbol table and the
"top" index where the scope ends.
*/
...
{
  // First check if "system" exists in the string table
  int systemIdx = getIndex("system");

  if (systemIdx != -1)
  {
    // Open a block for the global scope

    // Add system as a class
    int systemStIdx = InsertEntry(systemIdx, CLASS, 0);
    if (systemStIdx != -1)
    {
      // Set predefined attribute for system
      SetAttr(systemStIdx, PREDE_ATTR, true);

      // Open a block for the class scope

      // Check and add readln if it exists
      int readlnIdx = getIndex("readln");
      int printlnIdx = getIndex("println");
      if (readlnIdx != -1 | printlnIdx != -1)
      {
        OpenBlock();
      }
      if (readlnIdx != -1)
      {
        int readlnStIdx = InsertEntry(readlnIdx, FUNC, 0);
        if (readlnStIdx != -1)
        {
          SetAttr(readlnStIdx, PREDE_ATTR, true);
          SetAttr(readlnStIdx, ARGNUM_ATTR, 1);
        }
      }
      // Check and add println if it exists
      if (printlnIdx != -1)
      {
        int printlnStIdx = InsertEntry(printlnIdx, FUNC, 0);
        if (printlnStIdx != -1)
        {
          SetAttr(printlnStIdx, PREDE_ATTR, true);
          SetAttr(printlnStIdx, ARGNUM_ATTR, 1);
        }
      }
      if (readlnIdx != -1 | printlnIdx != -1)
      {
    ...
    // how do I close the outter and keep the inner open?
    // open the inner, close the outter.
